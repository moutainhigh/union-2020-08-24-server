...
public class BaiduPayCallbackAction extends UActionSupport{
    ...
    public void payCallback(){
        ...
        try{
            ...
            if(user == null){
                Log.d("user is not exists of order:%s", orderID);
                return;
            }
            ...
            if(resultCode == 1){
                ...
                Log.d("baidu pay callback content result:%s", jsonStr);
                ...
                if(payResult != null && payResult.getOrderStatus() == 1){
                    ...
                    if(order.getProductName() != null && !order.getProductName().equals(payResult.getMerchandiseName())){
                        renderState(order.getChannel(), 91, "Product Name 不匹配");
                        return;
                    }

                    if(payResult.getExtInfo() == null || !payResult.getExtInfo().equals("")){
                        renderState(order.getChannel(), 91, "Ext Info 不匹配");
                        return;
                    }

                    if(!user.getChannelUserID().equals(payResult.getUID()+"")){
                        renderState(order.getChannel(), 91, "UID 不匹配");
                        return;
                    }

                    int realMoney = (int)(payResult.getOrderMoney() * 100);

                    if(realMoney < order.getMoney()){
                        Log.d("baidu:order:%s money not matched. local price:%s; remote price:%s", orderID, order.getMoney(), realMoney);
                        renderState(order.getChannel(), 91, "OrderMoney 不匹配");
                        return;
                    }

                    order.setRealMoney((int)(payResult.getOrderMoney() * 100));
                    order.setSdkOrderTime(TimeUtils.format_default(payResult.getBankDateTime()));
                    ...
                    Log.e("baidu: The pay result decode2Bytes error. The content is "+jsonStr);
                }
