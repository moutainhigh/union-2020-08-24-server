...
//			String baseStr = generateBaseString(channel.getCpAppKey(), token, System.currentTimeMillis() + "",
...
//
//					callback.onFailed(
//							channel.getMaster().getSdkName() + " verify failed. the post result is " + result);
//				}
//
//				@Override
	...
	public static String generateBaseString(String appKey, String token, String timestamp, String nonce) {
		...
		try {
			sb.append(OAUTH_CONSUMER_KEY).append("=").append(URLEncoder.encode(appKey, "UTF-8")).append("&")
					...
					.append("=").append(URLEncoder.encode(nonce, "UTF-8")).append("&").append(OAUTH_VERSION).append("=")
			...
			UHttpAgent.getInstance().get(url, headers, params, "UTF-8", new UHttpFutureCallback() {
				...
				public void completed(String content) {
					try {
						...
						OppoBriefUser user = (OppoBriefUser) JsonUtils.decodeJson(content, OppoBriefUser.class);
						if (user != null) {
							if (user.getResultCode() == 200) {
								callback.onSuccess(new SDKVerifyResult(true, ssoid, user.getUserName(), ""));
								return;
