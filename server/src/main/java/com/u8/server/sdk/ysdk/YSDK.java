...
public class YSDK implements ISDKScript {
    ...
    public void verify(final UChannel channel, String extension, final ISDKVerifyListener callback) {
        ...
        try {
            ...
            if (type == 3) { //游客模式
                callback.onSuccess(new SDKVerifyResult(true, openid, "guest-" + openid, ""));
                return;
            }
            ...
            String key = channel.getCpAppKey();//QQ appId

            if (type == 1) {
                fullUrl = channel.getChannelAuthUrl() + WX_AUTH_PATH;
                key = channel.getCpAppSecret();//微信appkey
            }
            params.put("sig", EncryptUtils.md5(key + time));

            UHttpAgent.getInstance().get(fullUrl, params, new UHttpFutureCallback() {
                @Override
