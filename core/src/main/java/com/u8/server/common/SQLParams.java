...
public class SQLParams {
    ...
    public void NOTIN(String key, Object[] values) {
        addParam(key, values, SQLCondition.NOTIN);
    }
    ...
    public Object[] getWhereValues() {
        ...
        for (SQLParamPair p : params) {
            if (p.condition != SQLCondition.IN && p.condition != SQLCondition.NOTIN) {
        ...
        public String toString() {
            ...
            switch (condition) {
                ...
                case LIKE:
                    ...
                    break;
                case IN:
                    String vals = getInValues();
                    if (!StringUtils.isEmpty(vals)) {
                        sb.append(key);
                        sb.append(" IN (").append(vals).append(")");
                    }

                    break;
                case NOTIN:
                    String vals2 = getInValues();
                    if (!StringUtils.isEmpty(vals2)) {
                        sb.append(key);
                        sb.append(" NOT IN (").append(vals2).append(")");
                    }
