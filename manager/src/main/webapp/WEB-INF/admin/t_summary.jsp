...
<div class="easyui-panel infoBar">
  ...
  <ul>
    <li>
      <span class="u8_item_combobox" style="margin-top: -3px;"><input id="gamesToday" type="text" class="easyui-combobox" style="width: 120px;height: 25px;" prompt="选择游戏" name="allgamesToday"  /></span>
      <span class="u8_item_combobox" style="margin-top: 5px;"><input id="channels" type="text" class="easyui-combobox" style="width: 120px;height: 25px;" prompt="选择渠道" name="allchannels" /></span>
    </li>
    <li class="infoBar1"><span>今日激活设备</span><strong id="todayDeviceNum">0</strong></li>
    <li class="infoBar5"><span>今日新增玩家</span><strong id="todayNewUserNum">0</strong></li>
    <li class="infoBar5"><span>今日活跃玩家</span><strong id="todayUserNum">0</strong></li>
    <li class="infoBar3"><span>今日付费玩家</span><strong id="todayPayedUserNum">0</strong></li>
    <li class="infoBar4"><span id="todayMoneyTitle">今日收入（元）</span><strong id="todayMoneyNum"><small class="USD"></small>0</strong></li>

  </ul>

</div>

<div class="easyui-panel infoBarWithTitle" title="历史统计数据">
...
<script type="text/javascript">
  ...
  $(function(){
    ...
    //showKeyChart("['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun','Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']", "[{name: 'Tokyo',data: [7.0, 6.9, 9.5, 14.5, 18.2, 21.5, 25.2, 26.5, 23.3, 18.3, 13.9, 9.6]}, {name: 'New York',data: [-0.2, 0.8, 5.7, 11.3, 17.0, 22.0, 24.8, 24.1, 20.1, 14.1, 8.6, 2.5]}, {name: 'Berlin',data: [-0.9, 0.6, 3.5, 8.4, 13.5, 17.0, 18.6, 17.9, 14.3, 9.0, 3.9, 1.0]}, {name: 'London',data: [3.9, 4.2, 5.7, 8.5, 11.9, 15.2, 17.0, 16.6, 14.2, 10.3, 6.6, 4.8]}]");
     ...
     function reqTodayDataForGame() {

         var appID = $("#gamesToday").combobox("getValue");
         if(!appID) {
           $.messager.show({
             title: '操作提示',
             msg: '请先选择一个游戏'
         ...
         $.post('<%=basePath%>/analytics/todaySummaryData', {appID:appID}, function(result){
             if (result.state == 1) {
                 showTodayBaseInfo(result.data)
             }else{
                 $.messager.show({
                     title: '操作提示',
                     msg: '操作失败'
      ...
      function reqTodayDataForChannel() {
          ...
          $.post('<%=basePath%>/analytics/todaySummaryData', {channelID:channelID}, function(result){
              if (result.state == 1) {
                  showTodayBaseInfo(result.data)
              }else{
                  $.messager.show({
...
//        });
    ...
    function showBaseInfo(data){
      ...
      function showTodayBaseInfo(data){

          $("#todayDeviceNum").html(data.deviceNum);
          $("#todayNewUserNum").html(data.newUserNum);
          $("#todayUserNum").html(data.userNum);
          $("#todayPayedUserNum").html(data.payedUserNum);
          $("#todayMoneyTitle").html(formatMoneyTitle(data.moneyNum));
          $("#todayMoneyNum").html("￥"+formatMoney(data.moneyNum));
      ...
      $("#gamesToday").combobox({
          ...
          onSelect:function(rec){
              ...
              $("#channels").combobox({
                  ...
                  textField:'showName',
                  onSelect:function(rec){
                      reqTodayDataForChannel();
                  }
              });

          },
          onLoadSuccess:function(){
              var data = $('#gamesToday').combobox('getData');
              $("#gamesToday").combobox('select',data[0].appID);
          }
      });
