...
public class OrderTaskManager {
    ...
    private static final long DELAY_MILLIS = 60000;      //每次延迟执行间隔,ms. 这里是1分钟
//    private static final int MAX_RETRY_NUM = 6;         //最多重试6次
    ...
    public void addOrder(UOrder order){
        ...
        OrderTask task = new OrderTask(order, 1000, 1);
        this.tasks.add(task);
            ...
            public void run() {
                try{
                    ...
                    while(isRunning){
                        ...
                        task.run();
                        if(task.getState() == OrderTask.STATE_COMPLETE){
                            Log.d("the order %s resend to game server success.", task.getOrder().getOrderID());
                        }else if(task.getState() == OrderTask.STATE_FAILED){
                            Log.e("the order %s resend to game server failed.", task.getOrder().getOrderID());
                        }
                    }
