...
public class UOrder implements Serializable{
    ...
    private static final long serialVersionUID = 4055272937981700239L;
    ...
    @Transient
    private String ip;              //临时变量， 不存入数据库

    public JSONObject toJSON(){
        JSONObject json = new JSONObject();
        json.put("orderID", orderID+"");
        json.put("appID", appID);
        UGame game = CacheManager.getInstance().getGame(this.appID);

        json.put("appName", game == null ? "":game.getName());
        json.put("channelID", channelID);
        json.put("subChannelID", subChannelID);

        UChannel channel = CacheManager.getInstance().getChannel(channelID);
        if(channel != null){
            UChannelMaster master = CacheManager.getInstance().getMaster(channel.getMasterID());
            json.put("channelName", master == null ? "":master.getMasterName());
        }else{
            json.put("channelName", "未知渠道");
        }
        ...
        json.put("stateName", stateName);
        json.put("channelOrderID", channelOrderID);
        json.put("extension", extension);
        json.put("createdTime", createdTime == null ? "" : TimeUtils.format_default(createdTime));
